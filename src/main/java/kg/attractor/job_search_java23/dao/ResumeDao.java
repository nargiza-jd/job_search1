...
public class ResumeDao {
    ...
    public void addResume(Resume resume) {
        String sql = "INSERT INTO RESUME (user_id, title, category, expected_salary, telegram, email, phone, facebook, linkedin, published) " +
                "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    ...
    public void updateResume(Resume resume) {
        String sql = "UPDATE RESUME SET title = ?, category = ?, expected_salary = ?, telegram = ?, email = ?, phone = ?, facebook = ?, linkedin = ?, published = ? WHERE id = ?";
